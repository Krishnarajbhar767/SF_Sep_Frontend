...
const API_BASE = "home/section2";

export default function Home3GridSection2Editor() {
    ...
    const handleSave = async () => {
        try {
            ...
            for (let i = 0; i < newItems.length; i++) {
                if (fileMap[i]) {
                    const uploaded = await uploadMedia([fileMap[i]], { name: "home3GridSection2" });

                    if (uploaded && uploaded.length > 0) {
                        newItems[i].image = uploaded[0];
                    }
                }
                console.log('Nav Items ', newItems)
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto">
            <h1 className="text-2xl font-bold mb-4">Edit Home 3 Grid - Section 2</h1>
            ...
            {items.map((item, index) => (
                <div key={index} className="mb-4 border p-3 rounded">
                    {item.image && (
                        <img src={item.image} alt="item" className="w-32 h-20 object-cover mb-2 rounded" />
                    )}
                    <input
                        type="file"
